...
void dstd::vector<T>::reserve(unsigned int n)
{
	...
	unsigned int n_request = std::max<unsigned int>( this->capcity(), 2 );
	while( n_request < n )
	{
...
template <class T>
void dstd::vector<T>::insert(iterator position, iterator first, iterator last)
{
